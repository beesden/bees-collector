...
@Component({
  ...
  template: `
    ...
    <header>
      ...
      <p>
        <span *ngIf="collection.start">{{collection.start | date: 'yyyy'}}</span>
        <span *ngIf="collection.end">{{collection.end | date: 'yyyy'}}</span>
      </p>
    </header>
  `
})
export class SeriesCardComponent {
  ...
  @Input() collection: Series;

  constructor(private sanitizer: DomSanitizer) {
  }

  get image(): SafeStyle {
    return this.sanitizer.bypassSecurityTrustStyle(`url(${this.collection.image || this.defaultImage})`);
  }
